...
2. Understand the project — read in order:
   ...
   # installed package users: import { isStale } from "stale"; npx stale --help; npx stale-mcp
...
4. Verify:
   npm test  # must be 23 pass (8 isStale + 6 quote + 5 checkPrice + 4 MCP, mocked viem/MCP stdio, no live RPC)
...
6. Ready to PR:
   Create a new branch (not main), make your change in src/ (keep isStale pure, keep cre/lib in sync), keep 23 tests green (8 isStale + 6 quote + 5 checkPrice + 4 MCP), mock viem/MCP stdio, no live RPC in tests, never commit AGENTS.md/LINUS.md/.env/private keys, official Chainlink fields only, then open PR against main — opencode-review runs on PR.
