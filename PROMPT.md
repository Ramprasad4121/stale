...
2. Understand the project — read in order:
   README.md, skill.md, CONTRIBUTING.md, src/isStale.ts, src/quote.ts, src/check.ts, src/cli.ts, src/index.ts, cre/README.md, cre/workflows/stale/workflow.ts, cre/workflows/stale/config.staging.json (maxAge 3600) vs config.production.json (60), .github/workflows/test.yml
...
4. Verify:
   npm test  # must be 19 pass (isStale 8, quote 6, check 5, mocked viem, no live RPC)
...
6. Ready to PR:
   Create a new branch (not main), make your change in src/ (keep isStale pure, keep cre/lib in sync), keep 19 tests green, mock viem, no live RPC in tests, never commit AGENTS.md/LINUS.md/.env/private keys, official Chainlink fields only, then open PR against main — opencode-review runs on PR.
